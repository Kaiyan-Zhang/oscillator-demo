...
export const MIDDLE_C_FREQUENCY: number = 261.63;
...
export const eventKeyToSemitone: Record<string, number> = {
  1: 24, 2: 26, 3: 28, 4: 29, 5: 31, 6: 33, 7: 35,
  q: 12, w: 14, e: 16, r: 17, t: 19, y: 21, u: 23,
  a: 0,  s: 2,  d: 4,  f: 5,  g: 7,  h: 9,  j: 11,
};

export const keyboardLayouts: string[][] = [
  ["1", "2", "3", "4", "5", "6", "7"],
  ["q", "w", "e", "r", "t", "y", "u"],
  ["a", "s", "d", "f", "g", "h", "j"],
];

export const getFrequency = (semitone: number, semitoneShift: number = 0): number => {
  const totalSemitones = semitone + semitoneShift;
  return MIDDLE_C_FREQUENCY * Math.pow(2, totalSemitones / 12);
};

export const getSemitone = (key: string, semitoneShift: number = 0): number => {
...
export const getFullNoteName = (key: string, semitoneShift: number = 0): string | null => {
...
export const isNoteKey = (key: string): boolean => {
